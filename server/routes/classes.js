import express from 'express';

var knex = require('../db.js');

let router = express.Router();


function getDayNumber(weekday) {
	const dayNames = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
  for (let dayNumber in dayNames) {
		if (weekday === dayNames[dayNumber]) {
			return dayNumber;
		}
	}
}

function calcDayInterval(weekday, lastWeekday) {
  let diff = getDayNumber(weekday) - getDayNumber(lastWeekday);
	return(diff > 0 ? diff : 7 + diff)
}

router.post('/', (req, res) => {
	const { subject, number_of_weeks, selected_weekdays, weekdaysString, classDate, classMonth, classWeekday, classYear, classHour, classMinutes, classAMPM, classEndHour, classEndMinutes, classEndAMPM, students_string, first_names_string, teacher, timezoneOffset } = req.body;
	// timezoneOffset is not used for anything currently
	// Dates and times will be saved in the database in UTC timezone regardless of their origin, 
	//   then later converted to the proper local timezone in the client
  const addPMHours = (classAMPM == 'p.m.') ? 12 : 0;	
  let classHourAMPM = ((classHour === 12) ? 0 : classHour) + addPMHours;
	const classFirstDay = new Date(classYear, classMonth, classDate, classHourAMPM, parseInt(classMinutes, 10));
	let thisClassDay = new Date(classFirstDay.toLocaleString('en-CA'));
  // time is part of the 'date' column in timestamptz format
	let begintime = new Date(classFirstDay.toLocaleString('en-CA'));
	let endHour = classEndHour;
	  endHour = (classEndAMPM === "p.m.") ? endHour += 12 : endHour;
	  endHour = (classEndAMPM === "a.m." && endHour === 12) ? 0 : endHour;
	let hoursInterval = endHour - begintime.getHours();
  if (hoursInterval < 0) {
		hoursInterval = hoursInterval + 24;
	}
	let saveArray = [];
	let week_number = 0;
	let lastWeekday = ""; 
	let date = "";
  // Subsequent meetings are calculated based on selected weekdays and number of weeks
	// Rotate the list of weekdays as needed so that dates start with the correct first weekday
  let dayOrder = selected_weekdays;
	  if (dayOrder[0] !== classWeekday) {
      do {
        dayOrder.push(dayOrder.shift());
		  } while (dayOrder[0] !== classWeekday)
		}
	const numberOfMtgs = number_of_weeks * selected_weekdays.length;
  // Make an entry line for each meeting date
	do {
		week_number += 1;
		// classDayNumber += 1;
    for (let weekday of dayOrder)  {
			const dayInterval = calcDayInterval(weekday, lastWeekday);
			if ((week_number !== 1)||(weekday !== dayOrder[0])) {
        thisClassDay.setDate(thisClassDay.getDate() + dayInterval);
			}
	    let weekday_endtime = new Date(thisClassDay.toLocaleString('en-CA'));
	    weekday_endtime.setHours((thisClassDay.getHours() + hoursInterval), classEndMinutes);
      lastWeekday = weekday;
			// make a unique Date object so that each one doesn't equal the newest since it's call by reference
			const newClassDay = JSON.parse(JSON.stringify(thisClassDay));
      saveArray.push({ 
				subject: subject, students_string: students_string, first_names_string: first_names_string, weekday: weekday, class_datetime: newClassDay, endtime: weekday_endtime, week_number: week_number, number_of_weeks: number_of_weeks, selected_weekdays: weekdaysString, mtg_requester: teacher
  		});
    } // for (let weekday . . .
  } while (week_number < number_of_weeks);

  knex('session').insert(saveArray)

	   .then(msg => res.json({success: true, msg: msg }))
	   .catch (errors => res.status(500).json({ error: errors}));
});

router.get('/:teacher', (req, res) => {
	  const now = new Date();

  knex('session')
	  .where('mtg_requester', req.params.teacher)
		.andWhere('endtime', '>', now)
	  .orderBy('class_datetime', 'asc')

	.then( sessions => {
		res.json({ sessions });
	})
	.catch (errors => res.status(500).json({ error: errors}));

});

export default router;

